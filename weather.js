...
function paramsResponse(response, prevSearch = false) {
  ...
  var cityInfo = $(
    `<h5>${cityName} (${timeConvertion(time)}) <img src = ${iconImage}></h5>`
  ).addClass("cityInfo");
  var temp = $("<p>")
    .html("Temperature: " + currentTemperature + "°F")
    .addClass("temp");
  var humidity = $("<p>")
    .html("Humidity: " + currentHumidity + "%")
    .addClass("humidity");
  var windS = $("<p>")
    .html("Wind Speed: " + currentWindSpeed + "MPH")
    .addClass("windS");
